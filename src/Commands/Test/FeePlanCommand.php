...
class FeePlanCommand extends Command
{
    ...
    {
        $this->setName('plisio-php-sdk:test-fee-plan');
    }

    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        try {
            $feePlan = $this->interaction->getFeePlanByPsyscid(Currencies::TBTC);
            if (null !== $feePlan) {
                $output->writeln("<fg=green>Status: {$feePlan->getStatus()}</>");
            } else {
